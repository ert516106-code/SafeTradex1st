...
export default function OrderHistory() {
  ...
  useEffect(() => {
    base44.auth.me()
      .then(u => base44.entities.Transaction.filter({ user_email: u.email }, '-created_date', 50))
      .then(txs => {
        const trades = txs.filter(t => t.type === 'trade_win' || t.type === 'trade_lose');
      ...
      {transactions.map(tx => {
        ...
        return (
          <div key={tx.id} className="px-4 py-3 flex items-center gap-3">
            <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${isWin ? 'bg-emerald-50' : 'bg-red-50'}`}>
              {isLong
            ...
            <div className="text-right shrink-0">
              <p className={`font-bold text-sm ${isWin ? 'text-emerald-500' : 'text-red-500'}`}>
                {isWin ? '+' : '-'}{tx.amount?.toFixed(2)} USDT
              </p>
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${isWin ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'}`}>
                {isWin ? 'WIN' : 'LOSS'}
              </span>
            </div>
          </div>

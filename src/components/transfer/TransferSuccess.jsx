...
import { CoinLogo, getAccount, getCoin, formatAmount, useTransfer, AccountBadge } from "../../pages/Transfer";
    ...
    <div className="mx-auto flex min-h-screen max-w-lg flex-col items-center justify-center px-6 pb-10 text-center">
      ...
      <div className="relative mb-8 flex h-28 w-28 items-center justify-center">
        <span
          ...
          style={{ animation: mounted ? "trSuccessRing 1.6s ease-out infinite" : "none" }}
        />
        <span
          ...
          style={{ animation: mounted ? "trSuccessPop 0.5s cubic-bezier(0.16,1,0.3,1) both" : "none" }}
        >
      ...
      <div
        className="mt-6 flex w-full flex-col items-center gap-4 transition-all duration-500 delay-100"
        style={{ opacity: mounted ? 1 : 0, transform: mounted ? "translateY(0)" : "translateY(8px)" }}
      >
        <span className="flex items-center gap-2 text-[18px] font-bold text-white">
          <CoinLogo coin={coin} size={26} />
          {formatAmount(amount, 6)} {coin.symbol}
        </span>

        <div className="flex w-full items-center justify-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3.5">
          <span className="flex items-center gap-2">
            <AccountBadge account={fromAccount} size={28} />
            <span className="text-[13px] font-semibold text-white/80">{fromAccount.name}</span>
          </span>
          ...
          <span className="flex items-center gap-2">
            <AccountBadge account={toAccount} size={28} />
            <span className="text-[13px] font-semibold text-white/80">{toAccount.name}</span>
          </span>
        </div>
      </div>

...
import { TransferHeader, GlowCard, PrimaryButton, useTransfer, getCoin, getNetwork } from "../../pages/Transfer";

export default function TransferReview() {
  const navigate = useNavigate();
  const { draft } = useTransfer();
  ...
  if (!draft.type) {
    return (
      <div className="mx-auto flex min-h-full max-w-lg flex-col">
        <TransferHeader title="Review Transfer" />
        <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
          <p className="text-[14px] text-white/50">No transfer details found.</p>
          <button
      ...
      <div className="flex flex-1 flex-col gap-5 px-4 pb-8 pt-6 sm:px-6">
        <div className="flex flex-col items-center gap-2 py-2">
          ...
          <span className="rounded-full bg-white/5 px-3 py-1 text-[11.5px] font-semibold text-white/50">
            {draft.type === "internal" ? "Internal Transfer" : "External Transfer"}
          </span>
        </div>

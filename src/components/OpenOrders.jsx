...
function CountdownBadge({ remainingSeconds, totalSeconds }) {
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <span className="text-xs font-bold text-primary">{mm}:{ss}</span>
...
export default function OpenOrders() {
  ...
  // Re-render every second for countdown display
  useEffect(() => {
  ...
  if (orders.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-sm font-semibold text-muted-foreground">No open orders</p>
      ...
      {orders.map(order => {
        ...
        return (
          <div key={order.id} className="px-4 py-3 flex items-center gap-3">
            <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${isLong ? 'bg-emerald-50' : 'bg-red-50'}`}>
              {isLong
                ? <TrendingUp className="w-4 h-4 text-emerald-500" />
                : <TrendingDown className="w-4 h-4 text-red-500" />}
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-sm">{order.coin}/USDT</p>
              <p className="text-xs text-muted-foreground">
                {isLong ? 'Buy Long' : 'Sell Short'} · {order.period} · {order.amount.toLocaleString()} USDT
              </p>
              <p className="text-xs text-muted-foreground">
                Entry: {order.entryPrice.toLocaleString('en-US', { minimumFractionDigits: 2 })}
              </p>
            </div>
            <div className="shrink-0">
              <CountdownBadge remainingSeconds={remaining} totalSeconds={order.totalSeconds} />
              <p className="text-xs text-emerald-500 font-medium text-right mt-1">+{order.potentialWin} USDT</p>
            </div>
          </div>
        );
      })}
